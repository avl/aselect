...
async fn main() {
    ...
    //let mut smuggle_ref = smuggle.clone();

    {
        ...
        let temp = safe_select!(
            capture(state, server, new_conn, magic),
            acceptor(
                {
            ...
            accept_result(
                {
                    //*smuggle_ref.lock().unwrap() = Some(magic.get()?);
                    let server = server.get_some()?;
    ...
    //println!("Smuggled!: {:?}", (*smuggle.lock().unwrap()));
